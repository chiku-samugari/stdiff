  ...
  (with-route (node r)
              (let (tmp)
                (with-route (cur route) modified
                  (acond ((and (atom cur)
                               (multiple-value-bind (code detected)
                                 (retrieve-by-route base (route-normalize route))
                                 (and detected (equal cur code))))
                          (acond ((and (notany (papply (start-with (route-normalize route) (funcall _ :route))) tmp)
                                       (notany (papply (start-with (funcall _ :route) (route-normalize route))) tmp))
                                  (car (push (reserve-route route cur refmark) tmp)))
                                 ((remove-if-not (papply (start-with (route-normalize route) (funcall _ :route)))
                                                 tmp)
                                  (progn (mapc (papply (funcall _ :cancel)) it)
                                         (car (push (reserve-route route cur refmark) tmp))))
                                 (t next-level)))
                         ((multiple-value-bind (code detected)
                            (retrieve-by-route base (route-normalize route))
...
(defun detect-lost-subtree (routeref-diff base refmark lostmark)
...
(let* ((base '(lambda (x)
       ...
       (modified '(lambda (x)
                    (princ x)
                    (print (showdiff *first-impl* *second-impl* x)))))
  (detect-lost-subtree (rdiff base modified 'ref 1)
                              base
                              'ref
                              'lost))
...
(defun blue (x)
  (format t "<font color=\"#0000ff\"> ~a</font>" x))
  ...
  (let ((*standard-output* out))
    ...
    (let* ((base '(lambda (x)
           ...
           (modified '(lambda (x)
                        (princ x)
                        (print (showdiff *first-impl* *second-impl* x))))
           (newnode-detected-diff (rdiff base modified 'ref 2))
           (lost-detected-diff (lost-subtree-list newnode-detected-diff base 'ref 'lost)))
      (white "(")
      (with-route (cur route) (merge-lost newnode-detected-diff lost-detected-diff 'ref)
        (cond ((refnode-p cur 'ref) (white (retrieve-by-route base (route-normalize (drop cur)))))
              ((lostnode-p cur 'lost) (red (retrieve-by-route base (route-normalize (drop cur)))))
              ((atom cur) (green cur))
