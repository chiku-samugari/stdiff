...
(defun route-booking (mark rroute subtree &optional cont (birthplace rroute))
  (let ((valid? t) (recalculated? nil))
    (dlambda (:route () (route-normalize rroute))
             (:rroute () rroute)
             (:mark () mark)
             (:subtree () subtree)
             (:birthplace () (route-normalize birthplace))
             (:valid? () valid?)
             (:alive? () (or valid? recalculated?))
             (:resolve () (cond (valid?  (cons mark rroute))
                                (recalculated? (funcall (alambda (ch)
                                                        ...
                                                        subtree))
                                (t subtree)))
             (:cancel (&optional new-subtree)
              (setf valid? nil)
              (cond (cont (setf recalculated? t)
                          (setf subtree (funcall cont)))
                    (new-subtree (setf recalculated? t)
                                 (setf subtree new-subtree)))))))
...
(defun rawdiff (base modified refmark lostmark citedmark allowed-distance)
  (with-route (node r) (rdiff% base modified refmark lostmark citedmark allowed-distance)
    (cond ((functionp node) (if (funcall node :alive?)
                              (funcall node :resolve)
                              'dead))
          ((atom node) node)
          (t next-level))))
...
(defun collect-partly-bookings (route book)
  (remove-if-not #'(and (funcall a0 :valid?)
                        (start-with route (funcall a0 :route)))
                 book))

(defun check-booking (rroute basebook)
  (let ((route (route-normalize rroute)))
    (acond ((find-if #'(and (funcall a0 :valid?)
                            (start-with-or-equal (funcall a0 :birthplace) route))
                     basebook)
            (values it
                    (route-relation route (funcall it :birthplace))
                    (funcall it :mark)))
    ...
    (labels ((booking-as-ref (cited node cont &optional cur)
                 ...
                 (if booking
                   (case relation
                     ...
                     (:indirect (cond ((eq mark lostmark)
                                       (funcall booking :cancel
                                                (funcall (alambda (subnode subrroute)
                                                           (case (route-relation (route-normalize cited) (route-normalize subrroute))
                                                             (:direct (car (push (route-booking citedmark cur subnode nil subrroute) basebook)))
                                                             (:indirect (loop :for cnode :in subnode
                                                                              :for idx = 0 :then (1+ idx)
                                                                              :collect (self cnode (cons idx subrroute))))
                                                             (t (car (push (route-booking lostmark subrroute subnode) basebook)))))
                                                         (funcall booking :subtree)
                                                         (funcall booking :rroute)))
             ...
             (booking-as-lost (rroute node)
               (multiple-value-bind (booking relation mark)
                 ...
                 (if booking
                   (case relation
                     (:direct (cond ((eq mark citedmark) booking)
          ...
          (cond ((and (not (invalid-p mnode)) (not (invalid-p bnode)))
                 (acond ((equal mnode bnode)
                        ...
                        ((find-ref-candidates mnode rroute allowed-distance base)
                         (list (booking-as-lost rroute bnode)
...
(and
  (equal (rawdiff '(a b c) '(a b c) :ref :lost :cited 1)
         '(:REF 0))
  (equal (rawdiff '(a b c x) '(a b c) :ref :lost :cited 1)
         '((:REF 0 0) (:REF 1 0) (:REF 2 0) (:LOST 3 0)))
  (equal (rawdiff '(a b c) '(a b c y) :ref :lost :cited 1)
         '((:REF 0 0) (:REF 1 0) (:REF 2 0) (Y)))
  (equal (rawdiff '(a b c x) '(a b c y) :ref :lost :cited 1)
         '((:REF 0 0) (:REF 1 0) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a (b) c x) '(a c y) :ref :lost :cited 1)
         '((:REF 0 0) ((:LOST 1 0) (:REF 2 0)) ((:CITED 1 0) Y) (:LOST 3 0)))
  (equal (rawdiff '(a ((b)) c x) '(a b c y) :ref :lost :cited 2)
         '((:REF 0 0) ((((:CITED 1 0))) (:REF 0 0 1 0)) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a (w (z b x)) c x) '(a b c y) :ref :lost :cited 2)
         '((:REF 0 0)
           (((:LOST 0 1 0) ((:LOST 0 1 1 0) (:CITED 1 0) (:LOST 2 1 1 0)))
            (:REF 1 1 1 0))
           (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a (b) c x) '(a b c y) :ref :lost :cited 1)
         '((:REF 0 0) (((:CITED 1 0)) (:REF 0 1 0)) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a ((b)) c x) '(a b c y) :ref :lost :cited 1)
         '((:REF 0 0) ((:LOST 0 1 0)) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a b c x) '(a (((b))) c y) :ref :lost :cited 1)
         '((:REF 0 0) ((((B)))) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a b c x) '(a (((b))) c y) :ref :lost :cited 2)
         '((:REF 0 0) ((((B)))) (:REF 2 0) ((:LOST 3 0) Y)))
  (equal (rawdiff '(a b c x) '(a (((b))) c y) :ref :lost :cited 3)
         '((:REF 0 0) ((((:REF 1 0)))) (:REF 2 0) ((:LOST 3 0) Y))))
